...
| **`echoScanner.worker.ts`** | **Echo scanner OCR.** Runs the actual OCR (a small self-hosted tesseract.js pool, `public/tesseract/`) for the screen/video scanner. Purely mechanical — preprocesses (grayscale, contrast, 3x upscale) and recognizes text from header/stats-block crops it's given; doesn't import any echo game-data modules itself, so it only ever returns raw recognized strings. All name/stat matching and value snapping happens on the main thread in `src/scanner/parse.ts`. Message flow: `init` → `ready`; `recognizeCandidate` (header + stats bitmaps) → `candidateResult` (header + stats text) or `error`; `terminate`. See [scanner.md](./scanner.md). |
